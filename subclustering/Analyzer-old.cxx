...
// Original version
  ...
  /*cout<<fXIP<<"  "<<fYIP<<endl;
  cout<<fXIPPrev<<"  "<<fYIPPrev<<endl;
  cout<<XIntPointPrev[0]<<"  "<<XIntPointPrev[0]<<endl;
  cout<<XIntPointPrev[1]<<"  "<<XIntPointPrev[1]<<"\n"<<endl;*/
...
void Analyzer::ScaledTrack(const char* nometh2){
  ...
  //TH2F *weights=new TH2F(Form("h2%s",nometh2),Form("h2%s",nometh2),100,0,1000,100000,1e-8,1);
  for(int j=0;j<fnpixelx;j++){
    for(int l=0;l<fnpixely;l++){
      ...
      if(fTrack->GetBinContent(j,l)>0){
        ...
        if((X-fXIP)*(X-fXIP)+(Y-fYIP)*(Y-fYIP)<550)
        {   //If fwScal or z change a lot this might need tweeks
        ...
        else  fScaledTrack->SetBinContent(fScaledTrack->GetXaxis()->FindBin(X),fScaledTrack->GetYaxis()->FindBin(Y),0 );

        //weights->Fill((X-fXIP)*(X-fXIP)+(Y-fYIP)*(Y-fYIP),Z*exp(-( sqrt( (X-fXIP)*(X-fXIP)+(Y-fYIP)*(Y-fYIP) ) )/fwScal));
